...
# ================= ИНЛАЙН: 3 КАРТОЧКИ =================
async def cover_thumb_url(bot, file_id):
    # thumbnail_url для InlineQueryResultArticle должен быть публичной ссылкой,
    # а не file_id — строим её через getFile (её фетчит сервер Telegram, а не клиент).
    if not file_id:
        return None
    try:
        ...
        logger.error(f"get_file для thumbnail не удался: {e}")
        return None
...
async def inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    waifu_thumb = await cover_thumb_url(context.bot, covers.get("waifu"))
    results.append(InlineQueryResultArticle(
        id="waifu", title="💖 Ежедневная вайфу", description="Нажми, чтобы узнать свою",
        thumbnail_url=waifu_thumb,
        input_message_content=InputTextMessageContent("Нажми кнопку ниже, чтобы узнать свою вайфу дня"),
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("💖 Узнать свою вайфу", callback_data="reveal")]])))

    luck_thumb = await cover_thumb_url(context.bot, covers.get("luck"))
    results.append(InlineQueryResultArticle(
        id="luck", title="🎰 Испытать удачу", description="Ежедневный чек-ин",
        thumbnail_url=luck_thumb,
    ...
    stats_thumb = await cover_thumb_url(context.bot, covers.get("stats"))
    results.append(InlineQueryResultArticle(
        id="stats", title="📊 Статистика", description="Серия и баланс",
        thumbnail_url=stats_thumb,
...
async def reveal(q, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not first_today:
        ...
        if not char:
            await q.edit_message_text("⚠️ База пуста. Добавь вайфу через ➕ и дождись одобрения.")
            return
    ...
    if first_today:
        ...
        await q.edit_message_text(text, entities=ents, reply_markup=waifu_kb(REFRESH_PRICE))
        ...
        await q.edit_message_text(text, entities=ents, reply_markup=None)
...
async def refresh(q, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(text, entities=ents, reply_markup=waifu_kb(REFRESH_PRICE * m["round"]))
    st["messages"][key] = m
    save_state(st)
