...
async def reveal(q):
    ...
    await q.edit_message_media(media=media, reply_markup=waifu_kb(500))
    st.setdefault("messages", {})[msg_key(q)] = {
        "owner": uid, "owner_name": caller, "round": 1}
    save_state(st)
...
async def refresh(q):
    ...
    await q.edit_message_media(media=media, reply_markup=waifu_kb(500 * m["round"]))
...
async def add_submit(q, bot, st):
    ...
    try:
        await bot.send_photo(MOD_CHAT_ID, sess["arts"][0]["file_id"], caption=cap, reply_markup=kb)
...
async def approve(q, bot, st, req_id):
    ...
    save_state(st)
    # --- публикация в канал ---
    # Оригинальные ссылки из заявки — только для ознакомления, публикуются под постом.
    source_lines = [f"{i}. {a['url']}" for i, a in enumerate(arts, 1) if a.get("url")]
    cap = (f" имя: {char_name}\n источник: {title_name}\n id: #{char_id}\n"
    ...
    try:
        if len(arts) == 1:
            msg = await bot.send_photo(PUBLISH_CHANNEL, arts[0]["file_id"], caption=cap)
